 ...
 *
 * dgraph2-bulk — offline batch loader for the embedded dgraph2 DB.
 *
 ...
 * and emits ready-made on-disk shards. dgraph2 is single-node, so the
 ...
 * Typical use:
 *
 *	dgraph2-bulk --dir ./data --schema schema.dql --rdfs goldendata.rdf.gz
 *	dgraph2-bulk --dir ./data --json data.json --batch 1000
 *
 * The dgraph2-server must NOT be running on the same --dir at the same
...
func main() {
	dir := flag.String("dir", "./dgraph2-data", "data directory (must not be in use by dgraph2-server)")
