...
```sh
make build               # builds bonsai-server, bonsai-cli, bonsai-bulk, bonsai-live
...
```sh
./bonsai-cli alter 'name: string @index(exact) .'
./bonsai-cli mutate '_:a <name> "Alice" .'
./bonsai-cli query '{ q(func: eq(name, "Alice")) { uid name } }'
```
...
cmd/bonsai-cli/      gRPC + HTTP admin client
